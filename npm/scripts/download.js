...
// @dm-mcp/server postinstall
// 根据当前平台/架构，从 GitHub Releases 下载对应版本的 Go 二进制，
// 存到 bin/<platform>/ 下供 launcher (bin/dm-mcp.js) 调用。
// 版本来自 package.json（CI 发布时与 GitHub Release tag 对齐，如 v2.0.0）。
...
const REPO = 'mengqi1436/DM-MCP';

// 平台/架构 → 资产名映射（与 .github/workflows/release.yml 一致）
const ASSETS = {
  linux: {
    x64: 'dm-mcp',
  },
...
if (!asset) {
  ...
  console.warn(
    `[dm-mcp] 当前平台/架构 (${platform}/${arch}) 暂无可用二进制，跳过下载。支持: linux/x64, win32/x64`,
  );
  process.exit(0);
}
...
function download(url, redirectsLeft = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, { headers: { 'User-Agent': '@dm-mcp/server-install' } }, (res) => {
...
(async () => {
  try {
    ...
    console.log(`[dm-mcp] 已安装到 ${destPath} (${data.length} bytes)`);
  } catch (err) {
    console.error(`[dm-mcp] 二进制下载失败: ${err.message}`);
    console.error('[dm-mcp] 请确认网络可访问 GitHub，或设置 npm_config_ignore_scripts=true 跳过安装。');
    process.exit(1);
  }
})();
