...
- **版本**：2.0.0
- **代码位置**：`github.com/mengqi1436/DM-MCP`（v2 已取代 v1 成为仓库主代码）
- **目标环境**：达梦 DM8，本机 `SYSDBA/Sjc980514!@localhost:5236`（数据库 `DAMENG`）
...
1. **本地 HTTP 服务**：`dm-mcp.exe -transport=http -addr=:8090`，供支持 2026-07-28 streamable HTTP 的客户端连接（官方 SDK client / Codex 等）。
2. **stdio 本地进程**：`dm-mcp.exe -transport=stdio`，供本地编排器以子进程方式拉起（与 v1 相同接入方式）。
...
环境：`go test -bench=. -benchtime=1x dm-mcp/database`，DM_BATCH_SIZE=500。
...
| 验收项 | 结果 |
|---|---|
| `go build -o dm-mcp.exe`（amd64） | ✅ 16.6 MB |
| `go test ./...`（单元 + 集成） | ✅ 全部通过 |
...
1. **替换二进制**：`dm-mcp.exe` 替代 `dm-mcp.exe`（注意 amd64 架构）。
2. **环境变量**：
